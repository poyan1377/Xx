...
# NEW: Global dictionary to track terms acceptance status for mini app
TERMS_ACCEPTED = {}
...
# =====================================================================
# NEW: Callback handlers for Mini App Terms Acceptance
# =====================================================================
async def accept_terms_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer()
    user = update.effective_user
    TERMS_ACCEPTED[user.id] = True
    await query.edit_message_text(
         text=f"کاربر {user.first_name} شما قوانین را تایید کردید 🙂 هم‌اکنون می‌توانید حساب خود را شارژ کنید 🤩"
    )

async def reject_terms_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer("شما قوانین را رد کردید. امکان شارژ حساب وجود ندارد.", show_alert=True)
...
# NEW: Modified Charge Account Handler with Mini App Integration
async def charge_account(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    if not TERMS_ACCEPTED.get(user.id, False):
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("قوانین ربات ✅", web_app=WebAppInfo("https://poyan77.alfahost.sbs/xz.html"))]
        ])
        await update.message.reply_text(
            f"کاربر {user.first_name} لطفا ابتدا قوانین را تایید کنید سپس اقدام به شارژ حساب کنید ✅",
...
def generate_user_stats():
    ...
    lines.append(f"💳موجودی کل کاربران : *{total_balance}*")
    lines.append("➖➖➖➖➖➖➖➖➖➖")
...
async def trx_fixed_amount_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_first_name = update.effective_user.first_name
    message = (
        f"🌐کاربر {user_first_name} لطفاً مبلغ *{trx_amount:.4f}* TRX را به ولت زیر انتقال دهید و هش تراکنش را به پیوی ادمین ارسال کنی[...]\n"
...
if __name__ == '__main__':
    async def main():
        ...
        # Register Mini App Callback Handlers for Terms
        application.add_handler(CallbackQueryHandler(accept_terms_handler, pattern="^accept_terms$"))
        application.add_handler(CallbackQueryHandler(reject_terms_handler, pattern="^reject_terms$"))
